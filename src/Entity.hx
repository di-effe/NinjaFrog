class Entity {
	...
	public var allowJumpthrough : Bool;
    ...
    public function new(x:Int, y:Int) {
		...
		// setPosCase(x,y);
		// Replacing setPosCase because of a bug rendering y position with 1pixel off when adjacent to other entities
		setPosPixel( (x*Const.GRID)+Const.GRID/2 , (y*Const.GRID)+Const.GRID-1 );
	...
	function hitFromAbove(source, target, ?r = 0.8) {
		var distY = 1;
		// if( level.hasCollision(target.cx,target.cy+1) )
		// 	distY = 1;
		if ( source.cx == target.cx && source.cy == target.cy-distY && yr >= r && dy > 0) {
			return true;
		}
		else { 
			return false;
	...
	public function update() { // runs at an unknown fps
		...
		// Check higher position whole on the ground or ascending
		if( onGround || dy <= 0 ) 
			fallHighestCy = cy;
		...
		while( steps>0 ) {
			...
			// ID 3 = Platform_Brown
			allowJumpthrough = false;
			if (level.level.l_Collisions.getInt(cx,cy-1) == 3) {
				allowJumpthrough = true;
			}

			// Detect collision with ceiling
			if( level.hasCollision(cx,cy-1) && yr<0.5 && !allowJumpthrough) {
				yr = 0.5;
			...
			if( level.hasCollision(cx,cy+1) && yr>=1 ) {
				dy = 0;
				yr = 1;
				onLand(cy-fallHighestCy); // Check how severe was the fall
			}
